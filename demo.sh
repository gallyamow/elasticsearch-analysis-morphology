...
curl --header "Content-Type:application/json" -XPUT 'http://localhost:9200/rustest/_doc/4' -d '{"body": "Японские автомобили вновь заняли в США первые места в рейтингах"}' && echo
curl --header "Content-Type:application/json" -XPUT 'http://localhost:9200/rustest/_doc/5' -d '{"body": "Январский дефицит платежного баланса Японии превысил $5 млрд"}' && echo
curl --header "Content-Type:application/json" -XPUT 'http://localhost:9200/rustest/_doc/6' -d '{"body": "Японская корпорация Sony представила новый смартфон под названием Xperia Sola."}' && echo
curl --header "Content-Type:application/json" -XPOST 'http://localhost:9200/rustest/_refresh' && echo
echo "Should return 5"
curl --header "Content-Type:application/json" -s 'http://localhost:9200/rustest/_search?pretty=true' -d '{"query": {"query_string": {"query": "body:Япония"}}}'  | grep "_id"
echo "Should return 4, 6"
curl --header "Content-Type:application/json" -s 'http://localhost:9200/rustest/_search?pretty=true' -d '{"query": {"query_string": {"query": "body:Японский"}}}'  | grep "_id"
